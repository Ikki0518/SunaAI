...
export default function ClientChatPageColumnReverse() {
  ...
  // 自動スクロール関数（column-reverseの場合は上にスクロール）
          ...
          >
            {messages.length === 0 ? (
              ...
              <div className="max-w-3xl mx-auto px-6 py-8 w-full">
                {/* ローディング表示を最上部に（column-reverseなので実際は最下部） */}
                {loading && (
                ...
                {/* メッセージを逆順で表示（column-reverseなので実際は正順） */}
                {[...messages].reverse().map((msg, reversedIdx) => {
                  const originalIndex = messages.length - 1 - reversedIdx;
                  return (
